...
namespace ThueXeMay.Areas.Admin.Controllers
{
    public class blogsController : BaseController
    {
        ...
        [ValidateInput(false)]
        public ActionResult Create([Bind(Include = "id,title,head,content,IsActive")] blog blog, HttpPostedFileBase image)
        {
        ...
        public ActionResult Edit([Bind(Include = "id,title,head,content,IsActive,date")] blog blog, HttpPostedFileBase image)
        {
            if (image != null && image.ContentLength > 0)
            {
                string _fn = Path.GetFileName(image.FileName);
            ...
            {
                blog bikes = db.blogs.Where(i => i.id == blog.id).FirstOrDefault();
                blog.image = bikes.image;
            }
            if (ModelState.IsValid)
            {
        ...
        public ActionResult Comment(int Blogg =0)
        {
            ...
            var cmt = db.comments.Include(m => m.blogs);
            if (Blogg != 0)
            {
